...
public class EQ175CombineTwoTables {
}
/*
Table: Person
...
/*
Solution:
select Person.lastName,Person.firstName,Address.state,Address.city
from Person
left join Address
...
we can join these two tables to get the address information of a person.

Considering there might be no address information for every person,
...
on Person.PersonId = Address.PersonId
;
Note: Using the where clause to filter the records will fail
if there is no address information for a person because it will not display the name information.
 */

/*
2 Ways to write the same query using LEFT JOIN. Easy explanation

Explanation:
The question asks us to report the first name, last name, city, and state of each person in the Person table. If the address of a personId is not present in the Address table, report null instead.
