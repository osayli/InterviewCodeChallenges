...
You are given two integers, x and y, which represent your current location on a Cartesian
 ...
 represents that a point exists at (ai, bi). A point is valid if it shares
...
Of the valid points, [2,4] and [4,4] have the smallest Manhattan distance from your
...
1 <= x, y, ai, bi <= 104
 */
public class EQ1779FindNearestPointThatHasTheSameXOrYCoordinate {
    public static int nearestValidPoint(int x, int y, int[][] points) {
        ...
        for (int[] arr:points){
        if(arr[0]==x||arr[1]==y) {
            if((Math.abs(arr[0]-x)+Math.abs(arr[1]-y))<diff){
              result=idx;
              total=arr[0]+arr[1];
              diff=Math.abs(arr[0]-x)+Math.abs(arr[1]-y);
        }else if((Math.abs(arr[0]-x)+Math.abs(arr[1]-y))==diff){
